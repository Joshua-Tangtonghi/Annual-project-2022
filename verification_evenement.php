...
if ($_POST['point_open_dispo'] < 0) {
    header('location:evenements_admin.php?message=Le nombre de points open est inférieur à 0.&type=danger');
    exit;
}
...
if ( $date_debut_event < $date_actuelle) {
    header('location:evenements_admin.php?message=Erreur dans la date de debut.&type=danger');
    exit;
}
if ( $date_fin_event < $date_debut_event) {
    header('location:evenements_admin.php?message=Erreur dans les dates.&type=danger');
    exit;
}
...
$resultat = $req->fetch();
if($resultat){
    header('location:evenements_admin.php?message=Ce nom d\'association est déjà uilisé.&type=danger');
    exit;
}
...
if ($reponse) {
    header('location:evenements_admin.php?message=Evenements créé avec succès.&type=success');
    exit;
}else{
    header('location:evenements_admin.php?message=Echec lors de la creation de l\'evenement.&type=danger');
    exit;
}
?>
